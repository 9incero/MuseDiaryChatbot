...
def main():
    ...
    data['memory']=ConversationSummaryBufferMemory(
    llm=llm,                # 요약에 사용될 LLM
    return_messages=False,    # 메시지 객체 형태로 반환
    max_token_limit=3000     # 최대 토큰 제한 (대략적으로 조정 가능)
)
    response, data, option = execute_state('', data)
    print("AI: ",response)
    print("data: ", data)
    ...
    while 1: 
        ...
        response, data, option = execute_state(user_input, data)
        ...
        if data['state'] is None:
            with open('optiontest_memory_change_mid_want_X_lyrics_O_conversation_log.json', 'w', encoding='utf-8') as f:
